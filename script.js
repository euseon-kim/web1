document.addEventListener("DOMContentLoaded", function () {
    ...
    function onScroll() {
      ...
      if (scrollY >= sectionHeight && scrollY < 2 * sectionHeight) {
        globalTwo.style.transform = `translateY(-${Math.min(sectionTwoProgress * 100, 1000)}%)`; 
        globalOne.style.transform = `translateY(${Math.min(sectionTwoProgress * 100, 100)}%)`;
      }

      if (scrollY >= 2 * sectionHeight) {
        globalThree.style.transform = `translateY(-${Math.min(sectionThreeProgress * 100, 1000)}%)`; 
